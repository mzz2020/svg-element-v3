...
export default class CreateAxios {
  ...
  initHttpFn = async (params = {}, isFromData = false, config = { headers: {} }, url, method) => {
    ...
    this.instance.defaults.timeout = config.timeout || 50000
    ...
    this.instance.interceptors.response.use(
      ...
      error => {
        ...
        if (error && error.response) {
          ...
          this.store.commit('setError', { status: true, message: eMsg, type: 'error' })
        } else {
          this.store.commit('setError', { status: true, message: '系统错误', type: 'error' })
        }
        return Promise.reject(error)
      }
    )
